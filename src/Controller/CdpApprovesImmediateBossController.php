...
class CdpapprovesimmediatebossController extends AppController {
    ...
    public function initialize() {
        ...
        //$this->grupos = array_keys($this->Fifa->getImmediateBossGroups($this->Auth->user('group_id'), $this->Auth->user('id')));
        $this->query = $this->Fifa->Cdprequests()->Cdprequests_aprobacion_jefe();
    }
    ...
    public function index() {
        $cdprequests = $this->paginate($this->query);
    ...
    public function view($id = null) {
        ...
        $query = $this->query->where(array('Cdprequests.state' => 4, 'Cdprequests.id' => $id));
    ...
    private function action($state_in, $state_out) {
        ...
        if ($this->request->is(['patch', 'post', 'put'])) {
            ...
            //if ((in_array($cdprequest['group_id'], $this->grupos)) AND ( $cdprequest->state == $state_in)) {
            if ( $cdprequest->state == $state_in) {
                $this->request->data['state'] = $state_out;
