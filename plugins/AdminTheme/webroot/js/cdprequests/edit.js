...
function addItemsTable(add_item) {
    ...
    trash = '<a class="btn btn-danger trash" title="Eliminar Rubro"><i class="fa fa-fw fa-trash"></i></a>';
    //data = '<input type="hidden" class="itemsdata" name="itemsdata[]" value="'+JSON.stringify(add_item)+'">';
    data = "<input type='hidden' class='itemsdata' name='itemsdata[]' value='" + JSON.stringify(add_item) + "'>";
    $(".table-items tbody").append("<tr><td>" + add_item.itemstypes.name + "</td><td>" + add_item.classifications.name + "</td><td>" + add_item.items.item + " " + add_item.items.name + "</td><td>" + add_item.resources.name + "</td><td>" + tasks + "</td><td class='valueFormat'>" + add_item.value + "</td><td>" + trash + data + "</td></tr>");
